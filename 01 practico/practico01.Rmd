...
Con la excepción de *ggplot2*, en este curso no veremos estos paquetes, que tienen una popularidad creciente. En mi experiencia son muy eficientes para la ciencia de datos, pero el objetivo de este curso es solamente aplicar R al análisis estadístico en ciencias biológicas. Creo que es útil enseñar *tidyverse* en cursos de R de nivel intermedio y no introductorio, o con otras orientaciones. Coincido parcialmente en esto con la opiniones como [ésta](https://github.com/matloff/TidyverseSkeptic/blob/master/READMEFull.md#dpt) y [ésta](https://towardsdatascience.com/a-thousand-gadgets-my-thoughts-on-the-r-tidyverse-2441d8504433), que pueden resumirse en algunos puntos.    
* *tidyverse* vuelve más complejo el aprendizaje de **R** para quienes no poseen una base en programación.   
...
* R base ofrece pocas herramientas que deben combinarse para resolver un problema. Por el contrario, las funciones de *tidyverse* se cuentan por cientos, son sumamente eficaces (y en muchos casos resuelven problemas que requieren bastante programación en R), pero limitan la creatividad, uno de los puntos que quiero enfatizar.  
* *tidyverse* está sujeto a rápidos cambios, mientras que *R Base* es altamente conservado, permitiendo la ejecución de rutinas incluso varios años después de publicadas. Por ese motivo, lo encuentro más util para asegurar una ivnestigación reproducible.   
* La misma estabilidad de *R Base* lo vuelve una mejor elección para crear funciones, uno de los objetivos de este curso.
