...
Con la excepción de *ggplot2*, en este curso no veremos estos paquetes, que tienen una popularidad creciente. En mi experiencia de uso de R durante 15 años y dictado de cursos durante 12, no he encontrado motivo para promover su uso entre quienes nos dedicamos al análisis estadístico en ciencias biológicas, aunque creo que es útil conocerlo y enseñarlo en cursos de R de nivel intermedio y no introductorio (como este) y que son fundamentales en actividades donde el procesamiento de datos, en general previo al análisis, es indispensable. Coincido parcialmente en esto con la [opinión de Norm Matloff](https://github.com/matloff/TidyverseSkeptic/blob/master/READMEFull.md#dpt), que en resumen puede resumirse en estos puntos.  
...
1. Construya (o utilice si ya tiene disponible) una base de datos donde se encuentren variables categóricas, variables continuas y valores faltantes. 
  ...
  + Realice un subset utilizando lo niveles de la variable categórica (factor).   
  + Calcule la media para una variable continua de su set de datos luego de realizar el subset.   
